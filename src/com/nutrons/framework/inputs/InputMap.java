package com.nutrons.framework.inputs;

class InputMap {


  // Xbox360 Controller Map
  static final int XBOX_X_1 = 0;
  static final int XBOX_X_2 = 4;
  static final int XBOX_Y_1 = 1;
  static final int XBOX_Y_2 = 5;

  static final int XBOX_X = 3;
  static final int XBOX_B = 2;
  static final int XBOX_A = 1;
  static final int XBOX_Y = 4;

  static final int XBOX_START = 8;
  static final int XBOX_SELECT = 7;

  static final int XBOX_LEFT_BUMPER = 5;
  static final int XBOX_RIGHT_BUMPER = 6;
  static final int XBOX_LEFT_TRIGGER = 2;
  static final int XBOX_RIGHT_TRIGGER = 3;


  // LogitechF310 Controller Map
  static final int LOGITECH_X_1 = 0;
  static final int LOGITECH_X_2 = 4;
  static final int LOGITECH_Y_1 = 1;
  static final int LOGITECH_Y_2 = 5;

  static final int LOGITECH_X = 3;
  static final int LOGITECH_B = 2;
  static final int LOGITECH_A = 1;
  static final int LOGITECH_Y = 4;

  static final int LOGITECH_START = 8;
  static final int LOGITECH__SELECT = 7;

  static final int LOGITECH_LEFT_BUMPER = 5;
  static final int LOGITECH_RIGHT_BUMPER = 6;
  static final int LOGITECH_LEFT_TRIGGER = 2;
  static final int LOGITECH_RIGHT_TRIGGER = 3;
}
